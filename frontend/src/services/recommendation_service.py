...
def calculate_skill_gaps(student: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Calculate high-level skill gaps from the skills currently
    collected by the CareerAI assessment.

    This is a rule-based recommendation layer.
    It is separate from the placement ML model.
    """
    ...
    gaps.append(
        {
            ...
            "recommendation": (
                "Continue DSA and advanced problem solving."
                if coding_gap == 0
...
def get_career_roles(student: dict[str, Any]) -> list[str]:
    """
    Generate potential career directions using simple
    rule-based logic.
...
def get_learning_recommendations(
    ...
    """
    Generate learning recommendations from detected gaps.
    """
